...
`SCRAPER_DELAY_SECONDS` (по умолчанию 1.0) — пауза между запросами к сайтам:
чем больше кинотеатров на мониторинге, тем важнее не частить, чтобы не словить
блокировку. Спецпоказы (опера, TheatreHD, экскурсии) распознаются по названию и
скрыты из афиши по умолчанию — чекбокс «Спецпоказы» возвращает их в выдачу.
...
- Парсеры читают серверный HTML через httpx + BeautifulSoup и **не исполняют
  JavaScript**. Сейчас Афиша.ру и Москино отдают расписание в HTML, поэтому всё
  работает; если источник перейдёт на клиентский рендеринг, понадобится
  headless-браузер (например, Playwright) или его внутренний JSON-API.
- Прогон, собравший 0 сеансов по источнику, помечается в журнале как **ошибка**,
  а не «успех»: почти всегда это смена вёрстки или блокировка, а не реальное
  отсутствие сеансов. Коды 403/429/503 сразу поднимают `ParserBlocked`, попадают
  в журнал и уходят в повтор.
